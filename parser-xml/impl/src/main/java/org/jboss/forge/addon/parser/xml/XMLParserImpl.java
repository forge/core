...
{
   public InputStream toXMLInputStream(final Node node)
   {
      return new ByteArrayInputStream(toXMLByteArray(node));
   }

   public String toXMLString(final Node node)
   {
      return new String(toXMLByteArray(node));
   }

   public byte[] toXMLByteArray(final Node node)
   {
      try
      {
   ...
   public Node parse(final byte[] xml)
   {
      return parse(new ByteArrayInputStream(xml));
   }

   public Node parse(final String xml)
   {
      return parse(xml.getBytes());
   }

   public Node parse(final InputStream stream) throws XMLParserException
   {
      try
      {
         ...
         NodeImpl root = new NodeImpl(doc.getDocumentElement().getNodeName());
