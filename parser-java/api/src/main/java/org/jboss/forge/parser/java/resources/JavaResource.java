   ...
   protected synchronized List<Resource<?>> doListResources()
   {
      try
      {
         ...
         for (Member<?, ?> member : getJavaSource().getMembers())
         {
            ...
            {
               list.add(new JavaFieldResource(getResourceFactory(), this, (Field<? extends JavaSource<?>>) member));
            }
            else if (member instanceof Method)
            {
               list.add(new JavaMethodResource(getResourceFactory(), this, (Method<? extends JavaSource<?>>) member));
            }
            else
            {
         ...
         if (getJavaSource() instanceof JavaEnum)
         {
            ...
            {
               list.add(new EnumConstantResource(getResourceFactory(), this, (EnumConstant<JavaEnum>) e));
            }
         }
