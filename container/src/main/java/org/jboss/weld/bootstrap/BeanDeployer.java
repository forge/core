...
public class BeanDeployer extends AbstractBeanDeployer<BeanDeployerEnvironment>
{
   ...
   public BeanDeployer addClass(String className)
   {
      ...
      catch (ResourceLoadingException e)
      {
         ...
         log.info(" \\--> Missing type: "
                  + e.getCause().getMessage().replaceAll("(.*: L)(.*);", "$2").replaceAll("/", "."));
   ...
   public void processAnnotatedTypes()
   {
      ...
      {
         // fire event
         ProcessAnnotatedTypeImpl<?> event = containerLifecycleEvents.fireProcessAnnotatedType(getManager(),
                  annotatedType, getEnvironment().getAnnotatedTypeSource(annotatedType));
         // process the result
         if (event != null)
         {
            if (event.isVeto())
   ...
   protected void createClassBean(SlimAnnotatedType<?> annotatedType,
            Map<Class<?>, Set<SlimAnnotatedType<?>>> otherWeldClasses)
   {
      boolean managedBeanOrDecorator = !getEnvironment().getEjbDescriptors().contains(annotatedType.getJavaClass())
   ...
   protected void processBeanAttributes(Iterable<? extends AbstractBean<?, ?>> beans)
   {
      ...
      for (AbstractBean<?, ?> bean : vetoedBeans)
      {
         ...
         {
            SpecializationAndEnablementRegistry registry = getManager().getServices().get(
                     SpecializationAndEnablementRegistry.class);
   ...
   public void createNewBeans()
   {
      ...
      for (Entry<InternalEjbDescriptor<?>, EnhancedAnnotatedType<?>> entry : getEnvironment()
               .getNewSessionBeanDescriptorsFromInjectionPoint().entrySet())
      {
         InternalEjbDescriptor<?> descriptor = entry.getKey();
         createNewSessionBean(descriptor,
                  BeanAttributesFactory.forSessionBean(entry.getValue(), descriptor, getManager()), entry.getValue());
      }
   }
   ...
   public void registerCdiInterceptorsForMessageDrivenBeans()
   {
      ...
      for (InternalEjbDescriptor<?> descriptor : getEnvironment().getEjbDescriptors())
      {
         if (descriptor.isMessageDriven())
         {
            ...
            {
               InterceptionModelInitializer.of(getManager(),
                        classTransformer.getEnhancedAnnotatedType(descriptor.getBeanClass(), getManager().getId()),
                        null).init();
            }
            InterceptionModel<ClassMetadata<?>, ?> model = getManager().getInterceptorModelRegistry().get(
