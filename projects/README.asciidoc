...
BuildSystem services for custom project types::

If you wish to implement a custom project type in your addon, you will need to use the `BuildSystem` service API. Each time a method in the `ProjectFactory` is called, all available `BuildSystem` instances are queried in priority order until a valid project result is found.
+
`BuildSystem` implementations must also publish the `BuildSystemFacet` types that they provide. This is done via the `getProvidedFacetTypes()` method. `BuildSystemFacet` implementations may only be installed by the `BuildSystem` implementation that produces them. They will not be installed automatically by the `ProjectFactory`. 
+
[source,java]
----
public class CustomBuildSystem implements BuildSystem {
   ...
   public Project createProject(final DirectoryResource dir) {
      ...
      try {
         factory.install(project, CustomBuildSystemFacet.class);
      }
   ...
   public Iterable<Class<? extends BuildSystemFacet>> getProvidedFacetTypes() {
      return Arrays.asList(CustomBuildSystemFacet.class);
   }
